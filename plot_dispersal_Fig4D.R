...
#	legend.position="none",
	...
	T3D1<-c('T3D11','T3D12','T3D13','T3D14','T3D15','T3D16')
	T3D3<-c('T3D31','T3D32','T3D33')
	T3D5<-c('T3D51','T3D52','T3D53','T3D54','T3D55','T3D56')
	...
	All.S<-list( Bee,T1D1,T1D3,T1D5,T2D1,T2D3,T2D5,T3D1,T3D3,T3D5 )
...
#	sncm.out.B=fit_sncm(OTU_B[i,list.B], taxon=TAXA_B[list.B,], pool=NULL)
...
#	geom_text( aes( label=names2 ), size=2 )+
	...
	scale_y_continuous( limits=c( 0,1.1 ),breaks=c( 0,0.2,4.0,0.6,0.8,1 ) ) +
...
#	pdf("dispersal_bacteria_all.pdf",paper="A4" ,useDingbats=FALSE)
...
print("##############################")
	...
	Palette.phylum1 <-c("#EF5656","#F7A415","#88888a","#2BB065")
	gp$T3D31<-gp$T3D31+scale_color_manual( values=Palette.phylum1 )

#	pdf("dispersal_bacteria.pdf",useDingbats=FALSE)
#	gridExtra::grid.arrange(gp$T1D11,gp$T1D31,gp$T1D51, gp$T2D11,gp$T2D31,gp$T2D51, gp$T3D11,gp$T3D31,gp$T3D51,gp$Bee1,nrow=4, ncol=3)	
#	dev.off()
	...
	DT.T3D1<-DT$T3D11
	setnames(DT.T3D1, "y","T3D1")
	
	DT.T3D3<-DT$T3D31
	setnames(DT.T3D3, "y","T3D3")
	
	DT.T3D5<-DT$T3D51
	setnames(DT.T3D5, "y","T3D5")

	DT.T1=DT.T1D1[DT.T1D3][DT.T1D5]
	DT.T2=DT.T2D1[DT.T2D3][DT.T2D5]
	DT.T3=DT.T3D1[DT.T3D3][DT.T3D5]
			
	DT.all1=merge(DT.Bee, DT.T1, all=TRUE)
	DT.all2=merge(DT.T2, DT.T3, all=TRUE)
	DT.all=merge(DT.all1, DT.all2, all=TRUE)
	DT.all[is.na(DT.all)]<-0
	DT.all=merge(DT.all, DT.B.taxa)
	
#	DT.all$names1=ifelse(DT.all$Bee>=.8 | DT.all$T1>=.8, DT.all$ASV,"")
	...
	graph.T1=venn(list(ASV.T1D1, ASV.T1D3, ASV.T1D5, ASV.Bee), zcolor="style", snames=c("d1","d3","d5","bee"))
	...
	graph.T2=venn(list(ASV.T2D1, ASV.T2D3, ASV.T2D5, ASV.Bee), zcolor="style", snames=c("d1","d3","d5","bee"))
	attr.T2<-attr(graph.T2,"intersection")

	
	ASV.T3D1=DT.all[DT.all$T3D1>0.8]$ASV
	ASV.T3D3=DT.all[DT.all$T3D3>0.8]$ASV
	ASV.T3D5=DT.all[DT.all$T3D5>0.8]$ASV
	
	graph.T3=venn(list(ASV.T3D1, ASV.T3D3, ASV.T3D5), zcolor="style", snames=c("d1","d3","d5"))
	attr.T3<-attr(graph.T2,"intersection")
	
	ASV.T1=unique( c( ASV.T1D1,ASV.T1D3,ASV.T1D5 ) )
	ASV.T2=unique( c( ASV.T2D1,ASV.T2D3,ASV.T2D5 ) )
	ASV.T3=unique( c( ASV.T3D1,ASV.T3D3,ASV.T3D5 ) )
	
#	pdf("venn_bacteria.pdf",useDingbats=FALSE)
	graph.all=venn(list(ASV.T1,ASV.T2,ASV.T3,ASV.Bee),  zcolor="style", snames=c("T1","T2","T3","Bee"))
	attr.all<-attr(graph.all,"intersection")
#	dev.off()
...
#	sncm.out.B=fit_sncm(OTU_B[i,list.B], taxon=TAXA_B[list.B,], pool=NULL)
	...
	list1=c(attr.all$'Bee')
	list2=c(attr.all$'T1:Bee')
	list3=c(attr.all$'T1:T2:T3:Bee')
	list4=c(attr.all$'T1:T3')
	list5=c(attr.all$'T1:T2:T3')
	
	list6=c( list1,list2,list3,list4,list5)
	
	
	DT2$shape=ifelse( DT2$ASV %in% list1,"Bee",ifelse( DT2$ASV %in% list2, "T1B", ifelse( DT2$ASV %in% list3,"core","else" ) ) )
	
	DT2$names2=ifelse(DT2$ASV %in% list6 | DT2$y > .8 , DT2$Genus,"")
...
#	DT.all$shapes=ifelse(DT.all$ASV %in% c(V.attr$'Bee'), "Bee","All")
	
	pp2=ggplot(DT.all, aes(x=Bee,y=T1))
	p2=pp2+geom_jitter(aes(color=ColPhy), size=2)+
	geom_text(aes(label=names1), size=2)+
	scale_color_manual(values=Palette.phylum) +
	theme_bw()+theme_change
	
	DT.all$names2=ifelse(DT.all$T1>=.75 | DT.all$T2>=.75, DT.all$ASV,"")
	
	pp3=ggplot(DT.all, aes(x=T2,y=T1))
	p3=pp3+geom_jitter(aes(color=ColPhy), size=2)+
	geom_text(aes(label=names2), size=2)+
#	scale_color_manual(values=Palette.phylum) +
	theme_bw()+theme_change

	
#	pdf("dispersal_bacteria_cond.pdf",paper="A4" ,useDingbats=FALSE)
	gridExtra::grid.arrange(gp$Bee1,gp$T1D11,gp$T2D11,gp$T3D11, nrow=2, ncol=2)
#	dev.off()
	
#disp.B<-rbind(disp$Bee1,disp$T1D11,disp$T1D31,disp$T1D51,disp$T2D11,disp$T2D31,disp$T2D51,
#disp$T3D11,disp$T3D31,disp$T3D51)
#rownames(disp.B)<-c(paste0("B", rep(1:10,1)))
#colnames(disp.B)<-"m"
#disp.B<-data.frame(disp.B)
#disp.B$concat<-c("Bee","T1D1","T1D3","T1D5","T2D1","T2D3","T2D5","T3D1","T3D3","T3D5")
#disp.B$comp<-c("B","T1","T1","T1","T2","T2","T2","T3","T3","T3")
#disp.B$kgd<-c(rep.int("B",10))



	DT.B<-DT.B.m[,c("rn",..Day1)]
	DT.B$mean<-DT.B[,.(rowMeans(.SD,na.rm = T)),.SDcols = Day1]
	list.B=DT.B[DT.B$mean!=0]$rn
		
	print("Computing dispersal rates for Bee, T1 and T2 samples")	
	spp=OTU_B[i,list.B]
	N <- mean(apply(spp, 1, sum))
	p.m <- apply(spp, 2, mean)
	p.m <- p.m[p.m != 0]
	...
	disp<-as.numeric(coef(m.fit))
   	mci=as.numeric(coef(m.fit)-m.ci[1])
	print(paste0("disparsal rate = ",coef(m.fit)))
	...
	DT.mean=DT.B[,c('rn','mean')]
	DT2=merge(DT1, DT.mean, by.x="rn",by.y="rn")
	DT2$names1=ifelse(DT2$y > .75, DT2$Genus,"")
	DT2$names2=ifelse(DT2$y > .75, DT2$ASV,"")
	
	DT2$shapes=ifelse(DT2$ASV %in% c(V.attr$'Bee'), "Bee","All")	
	
	pp4=ggplot(DT2, aes(x=mean,y=y))
	p4=pp4+geom_point(aes(color=ColPhy, shape=shapes), size=4)+
	geom_text(aes(label=names2), size=2)+
	scale_color_manual(values=Palette.phylum) +
	theme_bw()+theme_change
